...
#     return None
#
# def judge_equivalent_wrapper(args):
#     """Wrapper function to run async judge_equivalent_async in a thread"""
#     llm, e1, sent1, e2, sent2 = args
#     return asyncio.run(judge_equivalent_async(llm, e1, sent1, e2, sent2))
#
# @csrf_exempt
...
#
#         # Get user and entities synchronously
#         try:
...
#         }
#
#         # Step 1: Build similar entity pairs
#         combined_pairs = set()
...
#             futures = [executor.submit(judge_equivalent_wrapper, task) for task in tasks]
#             for future in futures:
#                 try:
#                     result = future.result()
#                     print("auto resolution result:", result)
#                     if result:
...
# 全局线程池 (单例模式)
GLOBAL_THREAD_POOL = ThreadPoolExecutor(max_workers=50)  # 根据服务器配置调整
