...
#
# async def analyze_sentence_batch_async(sentences, user_id):
#     """Process a batch of sentences with their entity pairs"""
#     all_prompts = []
#     sentence_info = []
#
#     for sentence in sentences:
...
#             sentence_info.append((sentence, e1, e2))
#
#     if not all_prompts:
#         return []
#
...
#             batch_info = sentence_info[i:i + BATCH_SIZE]
#
#             # Get a fresh LLM instance for each batch to distribute API key usage
#             batch_llm = get_llm_instance()
#             response = await batch_llm.agenerate(batch_messages)
#
#             for generation_list, (sentence, e1, e2) in zip(response.generations, batch_info):
#                 for generation in generation_list:
#                     parsed_text = extract_json_from_markdown(generation.text)
#                     if parsed_text:
#                         try:
...
#                             results.append(final_result)
#                         except json.JSONDecodeError as e:
#                             logger.error(f"Error parsing JSON response: {e}. Raw response: {parsed_text}")
#                     else:
#                         logger.warning(f"Could not extract JSON from LLM response. Raw response: {generation.text}")
#     except Exception as e:
...
#
#
# def process_sentence_batch_sync_wrapper(sentences, user_id):
#     """Sync wrapper for batch processing"""
#     try:
#         loop = asyncio.new_event_loop()
#         asyncio.set_event_loop(loop)
#         return loop.run_until_complete(analyze_sentence_batch_async(sentences, user_id))
#     except Exception as e:
...
#         futures.append(GLOBAL_THREAD_POOL.submit(
#             process_sentence_batch_sync_wrapper, batch, user_id
#         ))
#
#     for future in futures:
#         try:
...
#         except Exception as e:
#             logger.error(f"Failed to get result for a batch: {e}")
#
#     return results
#
#
...
# @require_http_methods(["POST"])
# def extract_causal_relations(request):
#     """Main endpoint for causal relation extraction"""
#     try:
#         data = json.loads(request.body)
#         user_id = data.get("user_id")
#         if not user_id:
...
#         results = process_sentences(sentence_list, user_id)
#
#         created = []
#         for r in results:
#             if not isinstance(r, dict) or "result" not in r or not (rel := r["result"].get("causal_relationship")):
...
#
#             if cause_name == e1.name and effect_name == e2.name:
#                 start_entity, end_entity = final_e1, final_e2
#             elif cause_name == e2.name and effect_name == e1.name:
#                 start_entity, end_entity = final_e2, final_e1
#             else:
#                 continue
#
#             if start_entity == end_entity:
