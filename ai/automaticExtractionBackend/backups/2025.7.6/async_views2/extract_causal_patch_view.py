...
#         return None
#
# # Function to create an LLM instance for each user
# api_key_cycle = itertools.cycle(OPENAI_KEYS)  # Create an infinite cycle iterator
#
# def get_llm_instance(max_retries=3) -> ChatOpenAI:
#     """
...
#
#
# # Wrap the canonical_entity access in an async call to avoid blocking the event loop.
# async def get_canonical_entity(entity):
#     # Check if canonical_entity is a model and needs async access
#     return await asyncio.to_thread(lambda: entity.canonical_entity if entity.canonical_entity else entity)
#
...
#         # )
#         sentences = await asyncio.to_thread(
#             lambda: Sentence.objects.filter(user=user).prefetch_related("entities", "entities__construct").all()
#         )
#
#         is_exist = await asyncio.to_thread(sentences.exists)
#         # if not sentences.exists():
#         if not is_exist:
#             return JsonResponse({
#                 "status": "success",
...
#             }, status=200)
#
#         results = await asyncio.to_thread(process_sentences, sentences, user_id)
#
#         created = []
#         for r in results:
#             if not isinstance(r, dict) or "result" not in r or not (rel := r["result"].get("causal_relationship")):
...
#             # final_e1 = e1.canonical_entity if e1.canonical_entity else e1
#             # final_e2 = e2.canonical_entity if e2.canonical_entity else e2
#             # Make asynchronous calls to get canonical entities for e1 and e2
#             final_e1 = await get_canonical_entity(e1)
#             final_e2 = await get_canonical_entity(e2)
#
#             start_entity = None
#             end_entity = None
#
...
#                 continue
#
#             triple, created_flag = await asyncio.to_thread(
