...
#     # Step 1: 构建相似实体对集合
#     combined_pairs = set()
#     # 只需要用 openai 的来进行筛选；列表是为了以后有多个列表
#     for model_name in EMBEDDING_MODELS:
#         for i in range(len(all_ids)):
#             eid1 = all_ids[i]
#             vec1 = all_embeddings[eid1].get(model_name)
#             # 如果没有 embedding 数据，跳过
#             if not vec1:
#                 continue
#
#             # 有的话开始计算相似度分数
...
#
#         # 如果 LLM 判断这两个 entity 是相等的 （在彼此的 sentence 语境下都是相同的意思）
#         if ask_llm_if_equivalent(e1.name, sent1, e2.name, sent2):
#             uf.union(e1.id, e2.id)
#             confirmed.append((e1.name, e2.name))
#
#     # Step 3: 找到代表实体 {xxx: [], xxx:[]}
#     cluster_map = defaultdict(list)
#
#     # 一个 entity 相似的所有实体都应该放到一个列表中，比如 id5: [id6, id9, id11] 然后再把 id6, id9, id11 的转换成
#     """
#     entity map:
#     {id6: id5,
#     id9:id5,
...
#
#     # 每个原始的 entity 在合并后应该指向什么新的 entity
...
#             entity_map[eid] = kept
#
#     # Step 4: 更新 canonical_entity 字段
#     updated_canonicals = []
#     for eid, kept_id in entity_map.items():
#         # 完成映射的时候如果 id5: id5 忽略
#         if eid == kept_id:
#             continue
#         try:
...
#             continue
#
#     # Step 5: 替换 Triple 中的引用为 canonical
#     updated_triples = []
