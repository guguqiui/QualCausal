    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    s, e1, e2 = future_to_task[future]
                    print(f"❌ Future failed for pair ({e1.name}, {e2.name}): {e}")
        ...
        for r in results:
            ...
            if rel == "none" or not isinstance(rel, dict):
                print(f"ℹ️ No causal relationship for pair: {r['e1'].name}, {r['e2'].name}")
                continue
            ...
            # 排除 cause == effect（无论是 name 还是 id）
            if cause == effect or e1.id == e2.id:
                print(f"❌ Skipped due to identical cause and effect: {cause}")
                continue
            ...
            if cause == r["e1"].name and effect == r["e2"].name:
                ...
                print(f"⚠️ Direction mismatch in sentence ID {r['sentence'].id}:")
                print(f"    Model returned: cause='{cause}', effect='{effect}'")
                print(f"    Entities provided: e1='{r['e1'].name}', e2='{r['e2'].name}'")
                continue
...
"""
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for future in as_completed(futures):
                ...
                # if preferred == "1":
                if preferred == "1" and t2.pk:
                    t2.delete()
                # elif preferred == "2":
                elif preferred == "2" and t1.pk:
                    t1.delete()
                else:
                    # print(f"⚠️ Could not resolve causal loop between {t1.id} and {t2.id}")
                    print(
                        f"⚠️ Unresolved conflict: both directions exist between {t1.entity_cause.name} ↔ {t1.entity_effect.name}")

        print(f"✅ Total causal triples created: {len(created)}")
