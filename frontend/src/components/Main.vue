  ...
  <div class="container">
    ...
    <transition name="fade">
      <div 
        ...
        v-if="isInstanceContentVisible && selectedInstances.length > 0"
      >
...
export default {
  ...
  methods: {
    ...
    async fetchGraphData() {
      try {
        const nodesResponse = await axios.get(`${config.baseURL}/api/${this.userId}/nodes/`);
        const linksResponse = await axios.get(`${config.baseURL}/api/${this.userId}/links/`);
        ...
        this.processData(instances, nodes, links);
      } catch (error) {
    ...
    renderGraph() {
      ...
      this.simulation = d3.forceSimulation(this.nodes)
        ...
        .on("end", () => {
          ...
          console.log("Graph layout is fully loaded!");
        });
    ...
    async goToNodeView() {
      try {
        ...
        // 检查 selectedNodeName 是否存在
        if (!this.selectedNodeName && (!this.selectedInstances || this.selectedInstances.length === 0)) {
          console.error('No node or instance is selected.');
          alert('Please select a node or instance before navigating.');
          return; // 提前返回，避免后续逻辑执行
        }

        // 获取当前选中的节点名称
        const currentNodeName = this.selectedNodeName ||
            (this.selectedInstances[this.currentIndex] && this.selectedInstances[this.currentIndex].name);

        // 如果 currentNodeName 仍然为空，抛出错误
        if (!currentNodeName) {
          console.error('Current node name is not valid.');
          alert('The selected node name is invalid. Please try again.');
          return;
        }
        ...
        console.error('An error occurred while navigating to the node view:', error);
        alert('Failed to navigate to the node view. Please try again later.');
      }
    },

    onNodeClick(event, d) {
      if (this.graphLoading) {
        alert("The graph is still loading. Please wait a moment.");
        return;
      }
      if (blockIfPendingNewNode(this)) {
        return; // 阻止切换节点的操作
      }
      handleNodeClick(this, event, d);  // 如果加载完成，继续正常逻辑
    },
