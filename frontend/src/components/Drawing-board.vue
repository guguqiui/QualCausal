    ...
    <div class="main-container">
      ...
      <div style="position: fixed; bottom: 20px; right: 20px;">
        <div class="confirm-button-container">
          <button @click="goToIntroView">Confirm</button>
        </div>
      </div>
    </div>
  </div>
    ...
    this.canvas.on('object:modified', (e) => {
      ...
      if (obj?.type === 'group' && !obj.arrowType) {
        ...
        arrowGroups.forEach((arrowG) => {
          ...
          // console.log("fromGroupId", arrowG.fromGroupId)
          // console.log("toGroupId", arrowG.toGroupId)
  ...
  methods: {
    ...
    updateArrowByShapes(arrowGroup) {
      ...
      const newArrowGroup = new fabric.Group([line, triangle, arrowLabel], {
        ...
        fromGroup: fromG,
        toGroup:   toG,
        fromGroupId: fromG.id,  // 保持旧箭头的 id
        toGroupId: toG.id
      });
