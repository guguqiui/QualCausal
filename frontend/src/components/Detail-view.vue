...
import { prevInterview, nextInterview, updateQuestion} from '@/utils/templateUtils.js';handleSearch
...
export default {
  ...
  methods: {
    ...
    async fetchGraphData() {
      try {
        const nodesResponse = await axios.get(`${config.baseURL}/api/${this.userId}/nodes/source/${this.name}`);
        const sourceResponse = await axios.get(`${config.baseURL}/api/${this.userId}/nodes/name/${this.name}`);
        const linksResponse = await axios.get(`${config.baseURL}/api/${this.userId}/links/source/${this.name}`);
        ...
        let links = linksResponse.data;
        const sourceNode = sourceResponse.data;
        const instances = instancesResponse.data;

        const visitedNodes = new Set();
        visitedNodes.add(this.name);

        let queue = [...nodes]; 
        let depth = 0;
        const MAX_DEPTH = 10;

        const fetchChildNodes = async (parentNodes) => {
          const childRequests = parentNodes.map(async node => {
            if (visitedNodes.has(node.name)) return { childNodes: [], childLinks: [] };

            visitedNodes.add(node.name);
            const [childNodesResponse, childLinksResponse] = await Promise.all([
              axios.get(`${config.baseURL}/api/${this.userId}/nodes/source/${node.name}`),
              axios.get(`${config.baseURL}/api/${this.userId}/links/source/${node.name}`)
            ]);

            return { 
              childNodes: childNodesResponse.data, 
              childLinks: childLinksResponse.data 
            };
          });
          ...
          results.forEach(({ childNodes, childLinks }) => {
            allChildNodes = allChildNodes.concat(childNodes);
            allChildLinks = allChildLinks.concat(childLinks);
          });

          return { allChildNodes, allChildLinks };
        };

        while (queue.length > 0 && depth < MAX_DEPTH) {
          const { allChildNodes, allChildLinks } = await fetchChildNodes(queue);
          queue = allChildNodes;
          nodes = nodes.concat(allChildNodes);
          links = links.concat(allChildLinks);
          depth++;
        }
